...
class Sign(object):
    def __init__(self):
        self.db =pymysql.connect(db = 'mobishop', user = 'root', passwd = '')
    ...
    def insertmark(self,mark_accessoire,mark_telephone):
        
        try: 
            self.cursor.execute("INSERT INTO accessoire values('%s','%s')" % (mark_accessoire,mark_telephone))
            self.db.commit()
            print(f"insertion succesfully  {mark_accessoire} and {mark_telephone}")
            return 1
